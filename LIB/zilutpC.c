...
int zilutpC(csptr amat, double *droptol, int *lfil, double permtol,
	    int mband, ilutptr ilusch, int udiag)
{
/*---------------------------------------------------------------------- 
...
|--------------------------------------------------------------------*/
   ...
   if (rmax > 0) {
      ...
      ilusch->perm2 =  (int *) Malloc(rmax*sizeof(int), "ilutpC:5" );
      iperm = ilusch->perm2;
...
|--------------------------------------------------------------------*/
	 ...
	 if ( cabs(w[jj]) > drop5*tnorm ) {    /*  DROPPING IN L  */
#ifdef ILUTIME
msg_timer_start(&t25);
#endif
